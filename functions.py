...
def compute_structure(N,M,structure,r,xcyl,ycyl):
    ...
    xcyl: integer
        xcoordinate cylinder
    ycyl: integer
        ycoordinate cylinder
    ...
    if structure == "cylinder":
        ...
        boundary = np.zeros((N,M))
        boundary[int(N/2):int(N/2)+10 ,0:int(3*M/4)] = 1
        boundary[int(5*N/6):int(5*N/6)+10,0:int(3*M/4)] = 1
        boundary[int(4*N/6):int(4*N/6)+10,int(M/4):] = 1
...
def bounce_back(flow_density_new,flow_density_old,boundary,bounce):
    ...
    Description:
    -----------
    This function ensures the backward flow at the boundary positions of the fluid
...
def relaxation(flow_density,f_eq,tau):
    ...
    f_eq: array of size (N,M,9)
        array containing the equilibrium distribution functions ordered according to the e vector
    tau: float
        relaxation time
...
def init_velocity(maxv,N,M):
    ...
    Description:
    ------------
    This function computes the inflow velocity profile (laminar)
...
def rho_zou_he(flow_density,u)
        rho_boundary = (1/(1-u[0,:,0]))*(flow_density[0,:,0]+flow_density[0,:,3]+flow_density[0,:,7]+2*(flow_density[0,:,5]+flow_density[0,:,4]+flow_density[0,:,6]))
        return rho_boundary
        
        
def zou_he_boundary(flow_density,f_eq):
        flow_density[0,:,1] = flow_density[0,:,5] + f_eq[0,:,1] - f_eq[0,:,5]
        flow_density[0,:,2] = flow_density[0,:,6] + f_eq[0,:,2] - f_eq[0,:,6]
        flow_density[0,:,8] = flow_density[0,:,4] + f_eq[0,:,8] - f_eq[0,:,4]
        return flow_density
